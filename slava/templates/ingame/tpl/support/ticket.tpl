...
<div class="col-lg-6">
	...
	<div class="block">
		...
		{if('{status}' == '1')}
			<button class="btn btn-success mb-0" onclick="close_ticket('{id}')">Закрыть тикет</button>
		{/if}
		{if(is_worthy("l"))}
			<button class="btn btn-danger mb-0" onclick="dell_ticket('{id}')">Удалить тикет</button>
		{/if}
	</div>
	{/if}
</div>
		...
		<div id="add_ticket_answer">
			{if('{status}' == '1')}
				...
				<div class="smile_input_forum mt-3">
					<button id="send_btn" class="btn btn-primary" type="button" onclick="send_ticket_answer({id});">Отправить</button>
