<div class="table-row" id="sid{id}">
	<div class="row" data-target="#modal{id}" data-toggle="modal" {if('{army_enable}' == '2')}onclick="load_wstats({id},{server},'{authid}');load_mstats({id},{server},'{authid}');"{/if} >
		<div class="col-lg-2 with-description">
			<p>
				{place}
			</p>
			<span>Место</span>
		...
		<div class="col-lg-2 with-description with-icon">
		...
		{if(in_array({type}, array(1,2,3,4,5)))}
			{if(in_array({type}, array(1,2,3)))}
				{if('{type}' == '2')}
					<div class="col-lg-1 with-description">
						<p>
							{rank}
						</p>
						<span>Ранг</span>
							...
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tr>
										...
										<td>
											{if('{reset_stats}' == '1')}
											<button class="btn btn-primary btn-sm mr-2" onclick="reset_stats({id}, {server})">Обнулить статистику - {reset_stats_price} руб.</button>
											{/if}
											{if(is_worthy_specifically("h", {server}))}
											<button class="btn btn-primary btn-sm" onclick="dell_user_stats({id}, {server})">Удалить</button>
											{/if}
										</td>
									</tr>
									</tbody>
