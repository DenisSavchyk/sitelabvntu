<div class="table-row" id="bid{bid}">
	<div class="row" data-target="#modal{bid}" data-toggle="modal" title="Подробнее">
		...
		<div class="col-lg-3 with-description with-icon">
			...
			<p id="mute_end{bid}" class="text-{class}">
				{time}
			</p>
			<span>Окончание</span>
		</div>
					...
					<div class="table-responsive mb-0">
						<table class="table table-bordered">
							...
							<tr class="{disp}" id="unmute_btns{bid}">
								...
								<td>
									{if('{price}' != '0')}
									<button class="btn btn-primary btn-sm" onclick="buy_unmute({bid},{server});" title="Купить размут за {price} руб">
										Купить размут - {price} руб
									</button>
									{/if}
									<button class="btn btn-primary btn-sm {disp2}" onclick="close_mute('{server}','{bid}');" id="unmute_btn{bid}">
										Разблокировать
									</button>
								</td>
							</tr>
