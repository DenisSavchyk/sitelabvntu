<div class="col-lg-6">
	...
	<div class="block ban-application ban-information">
		...
		<button onclick="search_ban_application({server_id})" type="button" class="btn btn-primary mb-0">Подробнее</button>
		...
		<button onclick="close_ban({id},'1','{bid}');" type="button" class="btn btn-success mb-0">Разбанить</button>
		<button onclick="close_ban({id},'2','{bid}');" type="button" class="btn btn-warning mb-0">Отказать</button>
		{/if}
		{if(is_worthy_specifically("o", {server_id}))}
		<button onclick="dell_ban({id});" type="button" class="btn btn-danger mb-0">Удалить</button>
		{/if}
	</div>
	{/if}
</div>
<div class="col-lg-6">
	<div class="block">
		...
		<div class="noty-block info with_code mt-1">{text}</div>
		...
		{if('{allow}' == '1')}
			<div id="add_new_comments">
				...
				<div class="smile_input_forum mt-3">
					<button id="send_btn" class="btn btn-primary" type="button" onclick="send_ban_comment({id});">Отправить</button>
