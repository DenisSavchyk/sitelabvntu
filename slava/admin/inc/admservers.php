<?php 
	...
	{
		$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_servers` WHERE `id` = '".abs( ( int ) $_POST['d_id'] )."' LIMIT 1" );
		$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `server_id` = '".abs( ( int ) $_POST['d_id'] )."'" );
		$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_tarif_time` WHERE `server_id` = '".abs( ( int ) $_POST['d_id'] )."'" );
		echo $eng->alert_mess('Сервер успешно удален!');
	}
	
	if ( isset( $_GET['server'] ) )
    {
		$query_server = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_servers` WHERE `id` = ".abs( ( int ) $_GET['server'] )." LIMIT 1" );
		
		if ( $db->n_rows( $query_server ) > 0 )
		{
			if ( ! empty( $_POST['red_server'] ) )
			{
				$sql_uniq = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_servers` WHERE `name` = '".$db->m_escape( trim( $_POST['red_name'] ) )."' AND `hostname` = '".$db->m_escape( trim( $_POST['red_hostname'] ) )."' AND `login` = '".$db->m_escape( trim( $_POST['red_login'] ) )."' AND `password` = '".$db->m_escape( trim( $_POST['red_password'] ) )."' AND `id` != '".abs((int)$_GET['server'])."' LIMIT 1" );
				...
				if ( $db->n_rows( $sql_uniq ) > 0 ) {
				    ...
				    $sfile = !empty($_POST['skins_file']) ? $_POST['skins_file'] : '/cstrike/addons/amxmodx/configs/cm/custom_models.ini';
				    $ssave = !empty($_POST['skins_save']) ? $_POST['skins_save'] : "[n] [%n] [%ct] [%tt] [0]";
					$db->m_query( "UPDATE `".DBcfg::$dbopt['db_prefix']."_servers` SET `name` = '".$db->m_escape( trim( $_POST['red_name'] ) )."', `hostname` = '".$db->m_escape( trim( $_POST['red_hostname'] ) )."', `login` = '".$db->m_escape( trim( $_POST['red_login'] ) )."', `password` = '".$db->m_escape( trim( $_POST['red_password'] ) )."', `address` = '".$db->m_escape( trim( $_POST['red_address'] ) )."', `path` = '".$db->m_escape( trim( $_POST['red_path'] ) )."', `rules` = '".$db->m_escape( trim( $_POST['red_rules'] ) )."', `skins` = '".abs( ( int ) $_POST['skins'] )."', `skins_file` = '".$db->m_escape( trim( $sfile ) )."', `skins_save` = '".$db->m_escape( trim( $ssave ) )."' WHERE `id` = '".abs( ( int ) $_GET['server'] )."'" );
					echo $eng->alert_mess( 'Сервер успешно изменен!' ).'<meta http-equiv=\'refresh\' content=\'1; url=http://'.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI].'\' >';
				}
			}
			
			if ( ! empty( $_POST['delete_tarif'] ) )
			{
				$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `id` = '".abs( ( int ) $_POST['del_id'] )."'" );
				$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_tarif_time` WHERE `tarif_id` = '".abs( ( int ) $_POST['del_id'] )."'" );
				echo $eng->alert_mess('Услуга успешно удалена!');
			}
		
			$date = $db->f_arr( $query_server );
			$save = !empty($date['skins_save']) ? $date['skins_save'] : '[n] [%n] [%ct] [%tt] [0]';
			echo '<script src="'.$url.'templates/standart/dist/css/skins3.js"></script>
					...
					<div class="box-body">
						<form role="form" action="" method="POST" autocomplete="off">
							...
							<div class="form-group">
								...
								<input type="password" id="red_password" name="red_password" value="'.$date['password'].'" placeholder="FTP Пароль" class="form-control" required>
							</div>
							<div class="form-group">
								<label>IP:PORT:</label>
							...
							<hr>
                            <div class="form-group">
                                <input type="checkbox" id="skinstwo" name="skins" value="'.$date['skins'].'">
                                <label for="skinstwo"> Включить продажу моделей?</label>
                                <span id="fileskin" class="pad" style="display: none;">
                                    <label>Путь к файлу скинов: <span class="text-red">С ФАЙЛОМ*</span></label>
                                    <input type="text" id="skins_file" name="skins_file" placeholder="Путь к файлу моделей с файлом" value="'.$date['skins_file'].'" class="form-control" disabled="">
                                    ...
                                    <input type="text" id="skins_save" name="skins_save" placeholder="Вид сохранения в файл" value="'.$save.'" class="form-control" disabled>
                                </span>
                            </div>
                            <hr>
							<div class="form-group">
								<label>Правила:</label>
								<textarea name="red_rules" class="form-control" id="editor1" style="resize:vertical;min-height: 400px;" placeholder="Правила сервера">'.$date['rules'].'</textarea>
							</div>
							<div class="clearfix">
			...
			if ( isset( $_GET['tarif'] ) )
			{
				$query_tarif = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `id` = '".abs( ( int ) $_GET['tarif'] )."' AND `server_id` = '".abs( ( int ) $_GET['server'] )."' LIMIT 1" );

				if ( $db->n_rows( $query_tarif ) > 0 )
				{
					if ( ! empty( $_POST['changetarif'] ) )
					{
						$sql_uniq = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `name` = '".$db->m_escape( trim( $_POST['name'] ) )."' AND `server_id` = '".abs( ( int ) $_POST['server'] )."' AND `id` != '".abs( ( int) $_GET['tarif'] )."' LIMIT 1" );
						$sql_serv = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_servers` WHERE `id` = '".abs( ( int ) $_POST['server'] )."' LIMIT 1" );
						...
						if ( $db->n_rows( $sql_uniq ) > 0 ) {
							...
							$db->m_query( "UPDATE `".DBcfg::$dbopt['db_prefix']."_tarifs` SET `server_id` = '".abs( ( int ) $_POST['server'] )."', `name` = '".$db->m_escape( trim( $_POST['name'] ) )."', `access` = '".$db->m_escape( trim( $_POST['access'] ) )."', `info` = '".$db->m_escape( trim( $_POST['info'] ) )."', `binds` = '".$db->m_escape( trim( $_POST['binds'] ) )."' WHERE `id` = '".abs( ( int ) $_GET['tarif'] )."' LIMIT 1" );
							echo $eng->alert_mess( 'Услуга успешно изменена!' ).'<meta http-equiv=\'refresh\' content=\'1; url=http://'.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI].'\' >';
						}
					}
					...
					if ( ! empty( $_POST['add_time'] ) )
					{
						if ( ! preg_match( '/^(?:\d+,)*\d+$/', trim( $_POST['time_add'] ) ) || ! preg_match( '/^(?:\d+,)*\d+$/', trim( $_POST['price_add'] ) ) ) {
							...
							for( $i = 0; $i < $n; $i++ )
							{
								$db->m_query( "INSERT INTO `".DBcfg::$dbopt['db_prefix']."_tarif_time` (`id`, `tarif_id`, `server_id`, `price`, `time`) VALUES (NULL, '".abs( ( int ) $_GET['tarif'] )."', '".abs( ( int ) $_GET['server'] )."', '".abs( ( int ) $arr_price[$i] )."', '".abs( ( int ) $arr_time[$i] )."')" );
							}
					...
					if ( ! empty( $_POST['time_change'] ) )
					{
						if ( ! preg_match( '/^[0-9]+$/', abs( ( int ) $_POST['time'] ) ) || ! preg_match( '/^[0-9]+$/', abs( ( int ) $_POST['price'] ) ) ) {
							...
							$db->m_query( "UPDATE `".DBcfg::$dbopt['db_prefix']."_tarif_time` SET `time` = '".abs( ( int ) $_POST['time'] )."', `price` = '".abs( ( int ) $_POST['price'] )."' WHERE `id` = '".abs( ( int ) $_POST['id'] )."' LIMIT 1" );
					...
					{
						$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_tarif_time` WHERE `id` = '".abs( ( int ) $_POST['del_t_id'] )."' LIMIT 1" );
					...
					<div class="col-md-6">
						<div class="box box-success">
							...
							<div class="box-body">
								<form role="form" name="tarif_form" action="" method="POST" autocomplete="off">
									<div class="form-group">
										<label>Название:</label>
									...
									<div class="form-group">
										...
										<textarea name="info" class="form-control" id="editor2" style="resize:vertical;min-height: 400px;" placeholder="Информация">'.$date['info'].'</textarea>
									</div>
									<div class="form-group">
										<label>Бинды:</label>
										<textarea name="binds" class="form-control" id="editor3" style="resize:vertical;min-height: 200px;" placeholder="Бинды">'.$date['binds'].'</textarea>
									</div>
								...
								<div class="box-body table-responsive no-padding">
									<table class="table table-bordered">
										...
										<tbody>';
										$query_time = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_tarif_time` WHERE `tarif_id` = ".abs( ( int ) $_GET['tarif'] )." ORDER by `id`" );
						...
						<div class="box-body">
							<div class="box-body table-responsive no-padding">
								<table class="table table-bordered table-hover">
										...
										<tbody>';
										$query = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `server_id` = '".abs( ( int ) $_GET['server'] )."'" );
