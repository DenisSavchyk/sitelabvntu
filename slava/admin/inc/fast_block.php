<?php 
	...
	{
		$query_server = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_servers` ORDER BY `id`" );
		if ( $db->n_rows( $query_server ) > 0 )
		{
			while ( $date = $db->f_arr( $query_server ) ) { $eng->up_cfg ( $date['id'], $eng->g_cfg( $date['id'] ) ); }
			$query_admins = $db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_admins` WHERE `utime` < ".time()." AND `utime` != 0" );
		...
		$db->m_query( "OPTIMIZE TABLE `".DBcfg::$dbopt['db_prefix']."_admins`" );
		echo $eng->alert_mess( 'База данных успешно оптимизирована!' );
	} else if ( ! empty( $_POST['del_chat'] ) ) {
		$db->m_query( "DELETE FROM `".DBcfg::$dbopt['db_prefix']."_chat_mess`" );
		echo $eng->alert_mess( 'Сообщения чата успешно удалены!' );
	} else if ( ! empty( $_POST['del_ses'] ) ) {
		$db->m_query( "UPDATE `".DBcfg::$dbopt['db_prefix']."_admins` SET `hash` = '".$at->GenerateKey()."'" );
		echo $eng->alert_mess( 'Сессии успешно удалены!' );
	}
	...
	<button type="button" class="btn btn-small btn-block btn-warning" data-toggle="modal" data-target="#m_ses">Очистить сессии</button>
