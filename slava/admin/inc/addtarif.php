<?php 
	...
	{
		$sql_uniq = $db->m_query( "SELECT * FROM `".DBcfg::$dbopt['db_prefix']."_tarifs` WHERE `name` = '".$db->m_escape( trim( $_POST['name'] ) )."' AND `server_id` = '".abs( ( int ) $_POST['server'] )."' LIMIT 1" );
		...
		if ( $db->n_rows( $sql_uniq ) > 0 ) {
			...
			$insert_tarif = $db->m_query( "INSERT INTO `".DBcfg::$dbopt['db_prefix']."_tarifs` (`id`, `server_id`, `name`, `access`) VALUES (NULL, '".abs( ( int ) $_POST['server'] )."', '".$db->m_escape( trim( $_POST['name'] ) )."', '".$db->m_escape( trim( $_POST['access'] ) )."')" );
			$id_tarif = mysqli_insert_id( $database );

			for( $i = 0; $i < $n; $i++ )
			{
				$db->m_query( "INSERT INTO `".DBcfg::$dbopt['db_prefix']."_tarif_time` (`id`, `tarif_id`, `server_id`, `price`, `time`) VALUES (NULL, '".$id_tarif."', '".abs( ( int ) $_POST['server'] )."', '".abs( ( int ) $arr_price[$i] )."', '".abs( ( int ) $arr_time[$i] )."')" );
			}
			echo $eng->alert_mess( 'Услуга успешно добавлена!' ).'<meta http-equiv=\'refresh\' content=\'1; url=http://'.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI].'\' >';
		}
	}
		...
		<div class="box box-warning">
			...
			<div class="box-body">
				<form role="form" action="" method="POST" autocomplete="off">
					...
					<div class="form-group">
						<label>Время:</label>
						<input type="text" id="time" name="time" pattern="^(?:\d+,)*\d+$" placeholder="Время через запятую: Например 0,30,60,90" class="form-control" required>		
					</div>
					<div class="form-group">
						<label>Цена:</label>
						<input type="text" id="price" name="price" pattern="^(?:\d+,)*\d+$" placeholder="Цена через запятую: Например 2000,300,600,900" class="form-control" required>
					</div>
